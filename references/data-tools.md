...
| 脚本 | 负责 | 不负责 |
|---|---|---|
| `scripts/pokedb.mjs` | 属性、种族值、特性、招式参数、相性、**投入后的六项实数面板**、速度线、学习合法性 | 伤害数字 |
...
node pokedb.mjs stats   <名...>  [--format champs|sv] [--level 50]
                                 [--nature <性格>] [--evs 特攻32,速32] [--spread <预设>]
...
STATS lv50 Pokémon Champions 单打｜66能力点/单项上限32(1点=+1面板)｜iv=固定31 性格=有
STATS 喷火龙|Charizard|性格胆小(+速-攻)|投入 特攻32/速32(64/66)|HP153 攻93 防98 特攻161 特防105 速167
STATS 振翼发|Flutter Mane|~配置未知|HP130~162 攻67~117 防67~117 特攻139~205 特防139~205 速139~205
...
**铁律：投入后的六项实数值只能来自这里（或 `calc.mjs --stats`），禁止手算。**
1 点面板足以翻转先手或确定数，它是伤害计算的输入，错在这里比伤害错得更早。

```bash
node pokedb.mjs stats 喷火龙 --nature 胆小 --evs 特攻32,速32   # 显式投入
node pokedb.mjs stats 喷火龙 --spread 特攻速攻                  # 用 calc.mjs 的同名预设
node pokedb.mjs stats A B C D E F --format champs              # 一次六只，做阵容面板表
node pokedb.mjs stats 振翼发                                    # 配置未知 → 逐项上下界
```

- `--evs` 的键接受 `hp/atk/def/spa/spd/spe`，也接受中文 `HP/攻/防/特攻/特防/速`；
  写成 `特攻32,速32` 或 `spa=32,spe=32` 都行。
- `--nature` 接受**中文 / 英文 / 日文 / 常见俗称**（`胆小` = `Timid` = `おくびょう`；
  `保守`→内敛、`孤僻`→怕寂寞 等俗称也认）。认不出的名字会打 `~未知性格名(…)` 标记，
  **不会静默按无修正算**。
- `--spread` 与 `calc.mjs` 共用同一套预设（`物攻满`/`特攻速攻`/`物耐满`/…），
  两个脚本对同一配置给出的面板逐字相同——它们读的是同一份 `lib/rules.mjs`。
- 超单项上限或超总量：打 `~超出…` 警告并照常算出数字，**不静默截断**。

**`--nature`/`--evs`/`--spread` 一个都不给 = 配置未知**，逐项输出
「0 投入 + 降性格 ~ 单项满投入 + 加性格」的上下界。这是每一项**真实可达的包络**，
但 **六项不可能同时取上界**——总共只有 66 点。脚本会把这句话打在输出里，
转述给用户时别丢掉，否则上界会被读成某个真实存在的配置。
（这与 `calc.mjs` 对 `"spread": "unknown"` 的双基准是同一套语言，两处措辞保持一致。）
...
| `nature` | 性格名，**中/英/日/俗称皆可**（`胆小` / `Timid` / `おくびょう`）。认不出会打 `~未知性格名` 标记而非静默忽略 |
